...
import org.asky78.android.favorite.network.*;
...
public class NetworkSample extends Activity {
...
//		menuText.add("투명 Dialog");//10
		...
		menuSummary.add("파일을 다운로드 하고 진행과정을 보여줍니다.");//0
...
//		menuSummary.add("배경이 아무것도 없는 대화상자. 아이콘만 보이도록 세팅");//10
		...
		((TextView)findViewById(R.id.txt_common_sample_subject)).setText("네트워크 관련 예제들");
	...
	private OnItemClickListener listItemClickListener = new OnItemClickListener() {
		public void onItemClick(AdapterView<?> l, View v, int position, long id) {
			switch (position) {
			case 0:
				startActivity(NetworkFileDownload.class);//파일다운로드
				break;
			case 1:
				break;
			case 2:
