...
import org.asky78.android.favorite.animation.*;
...
public class AnimationSample extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		menuText.add("투명도 변경");//1
		menuText.add("크기 변경");//2
		menuText.add("투명도 변경 후 크기 변경");//3
		menuText.add("복합 애니메이션");//4
		menuText.add("연속 애니메이션");//5

		menuSummary = new ArrayList<String>();
		menuSummary.add("이미지가 서서히 나타납니다.");//1
		menuSummary.add("이미지가 줄어듭니다.");//2
		menuSummary.add("순차적으로 이미지가 서서히 밝아진 뒤 크기가 증가됩니다.");//3
	...
	private OnItemClickListener listItemClickListener = new OnItemClickListener() {
		public void onItemClick(AdapterView<?> l, View v, int position, long id) {
			switch (position) {
			case 0:
				showSample(MyAlphaAnimation.class);//투명도가 증가
				break;
			case 1:
				showSample(MyScaleAnimation.class);//축소
				break;
			case 2:
				showSample(MyAlphaScaleAnimation.class);//투명도 후 확대
				break;
			case 3:
				showSample(MyAlphaScaleTransRotateAnimation.class);//4가지 효과를 넣은 복합 애니메이션
				break;
			case 4:
				showSample(MyContinuousAnimation.class);//연속 재생 이미지
				break;
			case 5:
				break;
			case 6:
